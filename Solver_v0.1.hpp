...
public:
  ...
  Solver_v0(MatrixXt Ham, VectorXt Psi0, std::complex<T> i): Hamiltonian(Ham), Psi_0(Psi0), ii(i) {
    std::string usage ("See README.md for more info:\n"
                               "./NumericalIntegration </path/to/code/files(specifically the .py file)>");
    google::SetUsageMessage(usage);
  }
  virtual ~Solver_v0() noexcept {;}

  //Copy constructors - Move constructors are omitted because they are unlikely to be used
  Solver_v0(const Solver_v0& other): Hamiltonian(other.Hamiltonian), Psi_0(other.Psi_0), ii(other.ii){;}

  Solver_v0& operator=(const Solver_v0& other){
    Solver_v0 tmp(other);
    *this = std::move(tmp);
    return *this;
  }
  ...
  inline void plotSolution(MatrixXt& solverOutput, std::size_t num_runs, T step){
    ...
    Py_Initialize();
      ...
      try{
        ...
        std::ofstream pyplot (FLAGS_current_path + "/plot.py", std::ios::trunc);
